...
public class ActiveRecord<T extends Object> {
	private Class<? extends Object> clazz;

	public ActiveRecord(Class<? extends Object> clazz) {
		this.clazz = clazz;
	}
	...
	public long count() {
		return Long.parseLong(
				em().createQuery("select count(*) from " + clazz.getSimpleName() + " e").getSingleResult().toString());
	}

	public long count(String query, Object[] params) {
		return Long.parseLong(
				bindParameters(em().createQuery(createCountQuery(query, params)), params).getSingleResult().toString());
	}
	...
	public JPAQuery find(String query, Object... params) {
		Query q = em().createQuery(createFindByQuery(query, params));
		return new JPAQuery(createFindByQuery(query, params), bindParameters(q, params));
	}
	...
	public String findByToJPQL(String findBy) {
		...
		for (int i = 0; i < parts.length; i++) {
			...
			if (part.endsWith("NotEqual")) {
				...
				jpql.append(prop).append(" < ?").append(index++).append(" AND ").append(prop).append(" > ?")
						.append(index++);
