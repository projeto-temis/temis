...
public abstract class Relatorio extends Objeto implements IJuiaAction {
	...
	private String getFieldAlign(Field f) {
		FieldProps ann = f.getAnnotation(FieldProps.class);
		if (ann != null)
			return "text-align: " + ann.align().name().toLowerCase() + ";";
		return "";
	}
	...
	public int getColsForXL(Edit ann) {
		if (ann.colXL() > 0)
			return ann.colXL();
		if (ann.colL() > 0)
			return ann.colL();
	...
	public String getActionFieldValue(Object o) {
		if (o == null)
			return "";
	...
	public String gerarCabecalhoHtml() {
		...
		for (Field fld : ModeloUtils.getFieldList(this.getClass())) {
			Edit ann = fld.getAnnotation(Edit.class);
			if (ann != null) {
				...
				gerarCabecalhoHtmlCampo(b, getActionFieldName(fld), getActionFieldValue(o), getColsForXL(ann));
			}
		}
		return b.toString();
	}
...
//	    padrao = padrao.replace("size: A4 portrait;", "size: " + m.group("format") + ";");
	...
	private String formatar(LinhaDeRelatorio l, Field f) throws IllegalAccessException {
		Object o = f.get(l);
		if (o == null)
			return "";
		return o.toString();
